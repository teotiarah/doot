...
**Language, complete and final.** All 31 keywords, the full grammar in [03-grammar.md](03-grammar.md), the typechecker, structs, methods, tag-only enums, `match`, `with`, lambdas, `defer`. The front end — lexer, parser, AST — is specified in [10-frontend.md](10-frontend.md) and lands first, in the order set by [D067](01-decisions.md#d067), whose third step is the specification suite in [11-spec-tests.md](11-spec-tests.md) — the primary test layer, and the point at which every lexical and syntactic diagnostic becomes spec-testable. The four stages after it — resolver, typechecker, schema checker, route checker — are specified in [12-semantics.md](12-semantics.md) and land in the order set by [D102](01-decisions.md#d102), which puts `doot routes` before `doot check` for the same reason [D067](01-decisions.md#d067) put `doot fmt` before both.
